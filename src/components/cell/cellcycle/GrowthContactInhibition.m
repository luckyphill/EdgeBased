classdef GrowthContactInhibition < AbstractCellCycleModel
	...
	% Contact inhibition is implemented to stop cells from starting growth. When a cell
	% reaches the start of its growth phase the cell will only start growing
	% if the size is greater than gamma*S0, i.e. the
	% actual size relative to the target size is greater than the growthTriggerFraction
	...
	methods
		...
		function obj = GrowthContactInhibition(p, g, f, dt)
		...
		function newCCM = Duplicate(obj)
			...
			newCCM = GrowthContactInhibition(obj.meanPausePhaseDuration, obj.meanGrowthPhaseDuration, obj.growthTriggerFraction, obj.dt);
			newCCM.SetAge(0);
