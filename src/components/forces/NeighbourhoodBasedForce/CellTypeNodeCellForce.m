classdef CellTypeNodeCellForce < AbstractNodeElementForce
	...
	methods
		...
		function obj = CellTypeNodeCellForce(sra, srr, da, ds, dl, cellTypes, dt, usingPolys)
		...
		function AddNeighbourhoodBasedForces(obj, nodeList, p)
			...
			for i = 1:length(nodeList)
				...
				if obj.useNodeNodeInteractions
					...
					nList = p.GetNeighbouringNodes(n, max(obj.dLimit(cT1,:)));
				else
					eList = p.GetNeighbouringElements(n, max(obj.dLimit(cT1,:)));
					nList = Node.empty();
				end
				...
				if obj.useNodeNodeInteractions
					...
					for j = 1:length(nList)
						...
						if ( isa(n.cellList, 'NodeCell') && isa(n1.cellList, 'NodeCell') )
							if (n.cellList.sisterCell == n1.cellList)
								...
								if (n.cellList.GetAge() < n.cellList.CellCycleModel.growthPhaseDuration)

									dds = n.cellList.divisionSeparation;
									fr = n.cellList.CellCycleModel.GetGrowthPhaseFraction();
									ds = dds + fr * (ds - dds);
