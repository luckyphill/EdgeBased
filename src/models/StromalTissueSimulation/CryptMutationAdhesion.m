classdef CryptMutationAdhesion < LineSimulation
	...
	methods
		...
		function obj = CryptMutationAdhesion(p, g, b, f, sae, spe, nh, ch, wnt, mutM, seed)
			...
			% mutM = the mutation multiplier for the adhesion to the stroma
			...
			cellTypes = [epiCellType, mutEpiCellType, stromalCellType];
			att = [0,   0,     b;
				   0,   0,   mutM*b;
				   b, mutM*b,  0]; % No attraction between epithelial cells

			obj.AddNeighbourhoodBasedForce(CellTypeInteractionForce(att, repmat(b,3), repmat(dAsym,3), repmat(dSep,3), repmat(dLim,3), cellTypes, obj.dt, true));
			...
			pathName = sprintf('CryptMutationAdhesion/p%gg%gb%gsae%gspe%gf%gda%gds%gdl%galpha%gbeta%gt%ghw%gnh%gnr%gch%gwnt%gmut%g_seed%g/',p,g,b,sae,spe,f,dAsym,dSep, dLim, areaEnergy, perimeterEnergy, tensionEnergy, halfWidth, nh, nicheRadius, ch, wnt, mutM, seed);
